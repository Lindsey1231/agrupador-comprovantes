...
def formatar_resultado(nome_arquivo, cnpjs, cpfs, valores):
    """Formata o resultado conforme solicitado: (CNPJ/CPF; VALOR)"""
    # Pega o primeiro CNPJ/CPF encontrado
    documento = None
    if cnpjs:
        cnpj = list(cnpjs)[0]
        documento = f"{cnpj[:2]}.{cnpj[2:5]}.{cnpj[5:8]}/{cnpj[8:12]}-{cnpj[12:]}"
    elif cpfs:
        cpf = list(cpfs)[0]
        documento = f"{cpf[:3]}.{cpf[3:6]}.{cpf[6:9]}-{cpf[9:]}"
    
    # Pega o primeiro valor encontrado e formata
    valor_formatado = "N/A"
    if valores:
        valor = list(valores)[0]
        valor_formatado = f"{valor:,.2f}".replace(",", "X").replace(".", ",").replace("X", ".")
    
    return f"{nome_arquivo} ({documento if documento else 'N/A'}; {valor_formatado})"
...
def organizar_por_cnpj_e_valor(arquivos):
    ...
    # =============================================
    # NOVA SEÇÃO: VERIFICAÇÃO DOS ARQUIVOS (ADICIONADA CONFORME SOLICITADO)
    # =============================================
    st.write("### Resultados da Leitura")
    for arquivo in arquivos:
        ...
        # Mostra o resultado formatado conforme solicitado
        resultado = formatar_resultado(nome, cnpjs, cpfs, valores)
